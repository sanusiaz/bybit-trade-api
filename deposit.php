...
<body>
    <div class="container">
        ...
        <div class="inner_container" id="coinDetailsForm">
            ...
            <input type="text" id="amount" placeholder="Enter Deposit Amount" value="70.2368">
            <button onclick="fetchDepositDetails()">Get Deposit Details</button>
        </div>
        ...
        <div class="details" id="depositDetails">
            ...
            <button class="validate_transaction_btn" onclick="validateDepositTransactions()">Validate Transactions</button>

        </div>

    <a href="./withdraw.php" style="color: white; display: block; text-decoration: underline;">Go to withdraw.</a>
    ...
    <script>
        function goBackToDeposit() {
            ...
            $('#coinDetailsForm button').text('Get Deposit Details');
        ...
        function fetchDepositDetails() {
            ...
            if (!coin || !chain) {
                alert('Please enter both coin and chain');
                return;
            }
            let __data = {}
            __data['chain'] = chain
            ...
            $.ajax({
                url: './get_deposit_address.php',
                type: 'GET',
                data: __data,
                beforeSend: () => {
                    $('#coinDetailsForm button').text('Loading Address...');
                },
                error: function(err) {
                    console.error(err);
                },
                complete: function(res) {
                    ...
                    let __tag = response.data.tag
                    let __address = response.data.address
                    let __coin = response.data.coin
                    let __chain = response.data.chain
                    let __coinType = response.data.chainType
                    let __depositLimit = response.data.depositLimit
                    let __depositAmount = response.data.depositAmount

                    document.getElementById('address').innerText = __address;
                    document.getElementById('depositAmount').innerText = __depositAmount;
                    document.getElementById('tag').innerText = __tag !== "" ? __tag : 'N/A';
                    document.getElementById('limit').innerText = __depositLimit;
                    document.getElementById('depositDetails').style.display = 'block';
                    document.getElementById('coinDetailsForm').style.display = 'none';
